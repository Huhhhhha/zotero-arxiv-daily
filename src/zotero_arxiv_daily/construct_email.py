...
def get_badge(tag:str|None, label:str|None=None) -> str:
  if tag == "core":
  ...
  if tag == "frontier":
    text = label if label else "前沿探索"
    return '<span style="display:inline-block;font-size:12px;font-weight:bold;color:#ffffff;background-color:#1565c0;padding:2px 10px;border-radius:10px;margin-bottom:6px;">' + text + '</span><br>'
  return ''
...
def render_email(papers:list[Paper]) -> str:
    ...
    frontier_papers = [p for p in papers if p.tag == "frontier"]
    frontier_count = len(frontier_papers)
    if core_count or frontier_count:
        frontier_labels = [p.tag_label for p in frontier_papers if p.tag_label]
        if frontier_labels:
            topic_text = '、'.join(frontier_labels)
            header = (
                ...
                f'<b>{frontier_count}</b> 篇前沿探索（蓝标：{topic_text}）</div>'
            )
        else:
            header = (
                '<div style="font-family:Arial,sans-serif;font-size:14px;color:#555;padding:10px 14px;'
    ...
    for p in papers:
        ...
        title_html = get_badge(p.tag, p.tag_label) + p.title
